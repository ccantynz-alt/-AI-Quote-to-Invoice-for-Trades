...
<style>
  ...
  .business-name { font-size: 22px; font-weight: bold; color: #0e9f6e; }
  ...
  table.items th { background: #0e9f6e; color: #fff; padding: 8px 10px; text-align: left; font-size: 12px; }
  ...
  .totals .grand-total td { font-weight: bold; font-size: 15px; border-top: 2px solid #0e9f6e; }
  .payment-box { margin-top: 24px; padding: 16px; background: #ecfdf5; border: 1px solid #0e9f6e; border-radius: 6px; }
...
<div class="header">
  ...
  <div>
    ...
    <div style="text-align:right;margin-top:6px;">
      ...
      <div>Date: <?php echo esc_html( date( 'd M Y', strtotime( $invoice->created_at ) ) ); ?></div>
      <?php if ( $invoice->due_date ) : ?>
        <div>Due: <strong><?php echo esc_html( date( 'd M Y', strtotime( $invoice->due_date ) ) ); ?></strong></div>
      <?php endif; ?>
      <div style="margin-top:4px;">
        <span class="status-badge status-<?php echo esc_attr( $invoice->status ); ?>"><?php echo esc_html( $invoice->status ); ?></span>
      </div>
    </div>
  </div>
</div>
...
<table class="items">
  ...
  <tbody>
    <?php foreach ( (array) $invoice->line_items as $item ) : ?>
    <tr>
...
<table class="totals">
  <tr><td>Subtotal</td><td style="text-align:right">$<?php echo number_format( (float) $invoice->subtotal, 2 ); ?></td></tr>
  <tr><td>Tax</td><td style="text-align:right">$<?php echo number_format( (float) $invoice->tax_amount, 2 ); ?></td></tr>
  <tr class="grand-total"><td><strong>Total Due</strong></td><td style="text-align:right"><strong>$<?php echo number_format( (float) $invoice->total, 2 ); ?></strong></td></tr>
</table>

<?php if ( ! empty( $settings['bank_details'] ) ) : ?>
<div class="payment-box">
  <strong>Payment Details</strong><br>
  <?php echo nl2br( esc_html( $settings['bank_details'] ) ); ?>
</div>
<?php endif; ?>

<?php if ( ! empty( $settings['payment_terms'] ) ) : ?>
  <div style="margin-top:16px;font-size:12px;"><?php echo esc_html( $settings['payment_terms'] ); ?></div>
<?php endif; ?>
